...
平台和两款游戏首次加载时会在浏览器 `localStorage` 中生成并复用 `fish:anonymous-id:v1`。它是当前浏览器实例的随机标识，仅用于关联本机缓存和后续匿名统计，不代表登录账号；清除站点数据、更换浏览器或使用无痕模式后会生成新的标识。缓存不可用时游戏仍可正常运行。

平台首页统一承载游戏题图和已解锁关卡选择，并以弹出的日历式网格展示关卡：每 5 关用线框表示一个组别，普通/困难关卡用颜色区分。游戏目录通过参数化可用性配置控制状态，可用游戏优先展示，不可用游戏自动排在列表底部；不可用游戏会显示配置中的原因，并禁用关卡选择、开始按钮和题图入口。游戏链接使用 `?level=<index>` 直达所选关卡；各游戏根路径也会直接进入当前进度，不再提供独立开始页。同语义操作统一使用 Lucide 图标。
