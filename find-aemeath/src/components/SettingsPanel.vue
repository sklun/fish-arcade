...
<template>
  <div class="overlay" role="presentation" @click.self="$emit('close')">
    <section aria-labelledby="settings-title" aria-modal="true" class="settings-panel" role="dialog">
      <div class="panel-heading">
        <div><span class="hud__eyebrow">SYSTEM CONFIG</span>
          <h2 id="settings-title">设置</h2></div>
        <button aria-label="关闭设置" class="icon-button" title="关闭" type="button" @click="$emit('close')">
          <X :size="20"/>
        </button>
      </div>
      <button :aria-pressed="autoMark" aria-label="自动标记" class="setting-row" type="button" @click="$emit('toggleAutoMark')">
        <span><b>自动标记</b><small>找到目标后标记全盘排除格</small></span>
        <span :class="{ 'toggle--on': autoMark }" class="toggle"><Check v-if="autoMark" :size="15"/></span>
      </button>
