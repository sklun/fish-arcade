...
import {createLevelHref, type GameProgress, readGameProgress} from './progress'
...
const availabilityReason = (game: GameCatalogEntry): string =>
    game.availability.available ? '' : game.availability.note
...
<template>
  <div class="site-shell">
    <header class="topbar">
      <a aria-label="Fish Arcade 首页" class="brand" href="/">
        <span aria-hidden="true" class="brand__mark"><Gamepad2 :size="20"/></span>
    ...
    <main>
      <section aria-labelledby="catalog-title" class="catalog-heading">
        <div>
      ...
      <section aria-label="游戏列表" class="game-list">
        <article v-for="game in sortedGameCatalog" :key="game.id" class="game-entry">
          <div class="game-entry__copy">
            <div aria-hidden="true" class="game-entry__number">{{ game.index }}</div>
            ...
            <div class="level-actions">
              <section :aria-label="`${game.title}当前关卡`" class="level-selector">
                <button
                ...
                <div v-if="expandedLevels[game.id]" :id="`${game.id}-level-picker`" class="level-selector__popover">
                  <div aria-label="难度颜色" class="level-calendar__legend">
                    <span><i aria-hidden="true" class="difficulty-dot"></i>普通</span>
                    <span><i aria-hidden="true" class="difficulty-dot difficulty-dot--hard"></i>困难</span>
                  </div>
              ...
              <a v-if="game.availability.available" :href="gameHref(game)" class="play-button">
                <Play :size="20" aria-hidden="true" fill="currentColor"/>
                {{ progressByGame[game.id].completedLevels > 0 ? '继续游戏' : '开始游戏' }}
              </a>
              <button v-else class="play-button play-button--disabled" disabled type="button">
          ...
          >
            <img :alt="game.imageAlt" :src="game.image"/>
